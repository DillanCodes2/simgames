...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    mode.add_argument(
        "--play-gui",
        action="store_true",
        help="(Interactive) Play a match with a simple PyGame GUI (click/space to roll; GUI prompts for buy/auction).",
    )
    ...
    # -------------------- PLAY (PYGAME GUI) MODE --------------------
    if getattr(args, "play_gui", False):
        # Lazy import so batch users don't need pygame installed
        try:
            from .ui_pygame import PygameUI
            from .gui_controller import PygameHumanController
        except Exception as e:
            raise SystemExit(
                "PyGame GUI mode requires pygame. Install it (e.g., pip install pygame) and retry.\n"
                f"Import error: {e}"
            )
        ...
        if len(ai_list) != 3:
            raise SystemExit("(Play-GUI) --ai must specify exactly 3 strategies.")

        for s in ai_list:
            if s not in KNOWN_STRATEGIES:
                raise SystemExit(f"(Play-GUI) Unknown AI strategy '{s}'. Known: {sorted(KNOWN_STRATEGIES)}")

        human_zero = hi - 1

        # Build controllers list (human controller will be swapped in after UI exists)
        controllers = []
        ai_i = 0
        for seat in range(4):
            if seat == human_zero:
                controllers.append(StrategyController("Aggressive"))  # placeholder
            else:
                controllers.append(StrategyController(ai_list[ai_i]))
                ai_i += 1
        ...
        # Let UI know which seat is human (1-based in CLI; store 0-based in sim for convenience)
        sim.human_index = human_zero  # used by ui_pygame

        ui = PygameUI(sim, title="Monopoly (Phase 2 GUI)")
        # Route sim log messages into the GUI panel
        sim.event_sink = ui.push_log

        # Swap in the GUI human controller now that UI exists
        sim.controllers[human_zero] = PygameHumanController(ui, name=args.human_name)

        print("\nStarting GUI match!")
        print("Seats:")
        for i, c in enumerate(sim.controllers, start=1):
            # PygameHumanController has label(); fallback to class name
            label = c.label() if hasattr(c, "label") else c.__class__.__name__
        ...
        for _ in range(args.max_turns):
            sim.turn_count += 1
            for i in range(4):
                # keep the window responsive even during AI turns
                if not ui.pump():
                    winner = "QUIT"
                    break
                ...
                if i == human_zero:
                    ui.request_roll(f"{p.name}'s turn: click ROLL or press SPACE")

                sim.run_turn(i)

                active = [pl for pl in sim.players if pl.balance > 0]
                if len(active) <= 1:
                    winner_player = active[0] if active else max(sim.players, key=lambda x: x.balance)
                    winner = sim.strategies[sim.players.index(winner_player)]
                    break
        ...
        if winner is None:
            winner_player = max(sim.players, key=lambda x: x.balance)
            winner = sim.strategies[sim.players.index(winner_player)]

        ui.push_log(f"Game over! Winner: {winner}")
